function [xvals, yvals] = extract_all_data(dataType, dataChannel, completeFilePath)
...
if (strcmp(currentdir(end-4:end), '\Sort') && strcmp(completeFilePath(end-8:end-4), '-sort'))
    fileext = completeFilePath(end-3:end);
    currentdir = [currentdir(1:end-5) '\Raw\'];
    completeFilePath = [currentdir completeFilePath(1:end-9) fileext];
end

%completeFilePath = 'E:\Mitch\Dropbox\Ripple\Grapevine\MatlabSupport\SampleGrapevineData\testData.nev';
...
switch dataType            
    ...
    case {'LFP' 'Hi-Res' 'Raw' 'Analog 30k' 'Analog 1k'}
        ...
        if strcmp(dataType, 'Analog')
            ...
            if plotStatus; plot(analogInputDataTime_s,analogInputData_uV); xlabel('Time (s)'); ylabel ([dataType ' (uV)']); xlim([0 analogInputDataTime_s(end)]); end;
        end
        xvals = analogInputDataTime_s;
        yvals = analogInputData_uV;
