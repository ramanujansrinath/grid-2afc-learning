...
% [2012/09/05 ELB] The following method to retrieve where the digital
% event is found in a nev file is flawed.  This method does not return
% the event referenced by Index
%eventClass = fileInfo.MemoryMap.Data.Class(...
%    fileInfo.MemoryMap.Data.PacketID == 0);
% posEvent = find(bitget(eventClass,idx), Index);

% The cached 'PacketID' tell use where to find the desired entity
% "find" the first Index times that an entity has electrode 0 and
% then we use the 'end' of this array to find the desired digital event.
posEvent = find(fileInfo.MemoryMap.Data.PacketID==0, Index);
